...
struct Reading {
  ...
  float acidityIndex;
  float estimatedPh;
  // Ambient snapshot captured at boot — sent to cloud AI for environmental calibration
  float ambientCo2  = 415.0f;
  float ambientTemp = 25.0f;
  float ambientRh   = 50.0f;
  bool  valid = false;
};
Reading lastReading;
...
bool initSCD41() {
  ...
  // *** CRITICAL: Disable Automatic Self-Calibration (ASC) ***
  ...
  if (err != 0) {
    ...
    Serial.println("SCD41 Automatic Self-Calibration DISABLED (breath mode)");
  }
...
void captureBreath() {
  ...
  // Carry the ambient baseline snapshot for cloud AI calibration
  lastReading.ambientCo2   = ambient.valid ? ambient.co2  : 415.0f;
  lastReading.ambientTemp  = ambient.valid ? ambient.temp : 25.0f;
  lastReading.ambientRh    = ambient.valid ? ambient.rh   : 50.0f;
...
bool uploadReading(const Reading &r, const String &userId) {
  ...
  // Environmental calibration metadata for Cloud AI processing
  doc["ambient_co2_ppm"]   = r.ambientCo2;
  doc["ambient_temp_c"]    = r.ambientTemp;
  doc["ambient_rh_pct"]    = r.ambientRh;
